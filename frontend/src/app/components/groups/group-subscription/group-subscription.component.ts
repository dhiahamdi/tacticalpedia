...
@Component({
  ...
  styleUrls: ['./group-subscription.component.scss'],
...
export class GroupSubscriptionComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.groupService.getMyGroupSub({service : this.selected_service , topic : this.selected_topic , discipline : this.selected_discipline}).subscribe(data => {
      this.groups = data;
